...
// Variable para controlar el timeout de alertas
let loginAlertTimeout = null;
...
async function handleGoogleLogin() {
  ...
  if (!trimmedEmail.includes('@') || !trimmedEmail.includes('.')) {
    ...
    if (loginAlert) {
      ...
      if (loginAlertTimeout) clearTimeout(loginAlertTimeout);
  ...
  // Validar mayúsculas y cuenta de Google
  const hasUppercase = trimmedEmail !== trimmedEmail.toLowerCase();
  const isGmail = trimmedEmail.toLowerCase().endsWith('@gmail.com');

  // Si tiene ambos errores: mayúsculas Y no es Gmail
  if (hasUppercase && !isGmail) {
    const loginAlert = document.getElementById('loginAlert');
    if (loginAlert) {
      // Cancelar timeout anterior si existe
      if (loginAlertTimeout) clearTimeout(loginAlertTimeout);
      loginAlert.innerHTML = '<strong>Error:</strong> El email debe estar en minúsculas y debe ser una cuenta de Google (@gmail.com).';
      loginAlert.style.display = 'block';
      loginAlertTimeout = setTimeout(() => {
        loginAlert.style.display = 'none';
      }, 5000);
    } else {
      alert('El email debe estar en minúsculas y debe ser una cuenta de Google (@gmail.com).');
    }
    return;
  }

  // Si solo tiene mayúsculas
  if (hasUppercase) {
    const loginAlert = document.getElementById('loginAlert');
    if (loginAlert) {
      // Cancelar timeout anterior si existe
      if (loginAlertTimeout) clearTimeout(loginAlertTimeout);
      loginAlert.innerHTML = '<strong>Error:</strong> El email debe estar en minúsculas. Por favor, escribe todo en minúscula.';
      loginAlert.style.display = 'block';
      loginAlertTimeout = setTimeout(() => {
        loginAlert.style.display = 'none';
      }, 5000);
    } else {
      alert('El email debe estar en minúsculas. Por favor, escribe todo en minúscula.');
    }
    return;
  }
  ...
  if (!isGmail) {
    ...
    if (loginAlert) {
      // Cancelar timeout anterior si existe
      if (loginAlertTimeout) clearTimeout(loginAlertTimeout);
      loginAlert.innerHTML = '<strong>Error:</strong> Debes usar una cuenta de Google (@gmail.com) para continuar.';
      loginAlert.style.display = 'block';
      loginAlertTimeout = setTimeout(() => {
        loginAlert.style.display = 'none';
      }, 5000);
    } else {
      alert('Debes usar una cuenta de Google (@gmail.com) para continuar.');
    }
    return;
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async function (e) {
      ...
      // Validar que sea una cuenta de Google (@gmail.com)
      if (!email.trim().toLowerCase().endsWith('@gmail.com')) {
        showRegisterError('Debes usar una cuenta de Google (@gmail.com) para registrarte.');
        return;
      }
